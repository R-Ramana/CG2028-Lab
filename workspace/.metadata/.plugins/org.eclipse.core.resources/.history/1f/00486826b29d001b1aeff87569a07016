...
@R10 - N counter
@R11 - y_n
>>>>>>> 045a37e095fbcf6eabffa7c2e2a2646a5471a79a
...
@ write asm function body here
	@ R10 is N-counter, number of times to run the loop/function
	@ Ensures R0 is not modified
	MOV R10, R0

	@ b[0], post-indexing offset to get b1 value the next call
	LDR R8, [R1], #4
	
	@ b[0] * x_n
	MUL R11, R3, R8

	@ load start address of the static array to x_store, y_store
	LDR R4, =x_store
	LDR R5, =y_store
	...
	@ R2 is the address of a values
	@ divide by a[0] at the end
	ADD R2, #4

	LDR R7, [R5]
	@ update x_store, y_store first 4 bytes to their own address
...
circle:
	@ LSL 2 = multiply by 4, so N_Max * 4 number of bytes for x
	ADD R6, R4, R0, LSL #2
	
	@ LSL 2 = multiply by 4, so N_Max * 4 number of bytes for y
	ADD R7, R5, R0, LSL #2
...
zero:
	...
	@ get back start address of a to get a1
	@ R0, LSL 2 = N * 4
	SUB R2, R2, R0, LSL #2
	
	@ to get start address a0
	SUB R2, #4
	LDR R8, [R2]
...
.lcomm n_calls, 4
=======
@ each int = 32 bits = 4 bytes
@ store at most N_MAX integers = 4 * N_MAX (10) bytes
@ the first 4 bytes stores a pointer to (n-1)th value
.lcomm x_store, 44
.lcomm y_store, 44
>>>>>>> 045a37e095fbcf6eabffa7c2e2a2646a5471a79a
